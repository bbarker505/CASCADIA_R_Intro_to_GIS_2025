...
### **Interactive mapping of lilac leaf out data**
...
factors_df <- data.frame(
  ...
  leaf_out_doy = 1:365, 
  leaf_out_month = ggplot2::cut_interval(1:365, 12)) %>%
...
factors_df$leaf_out_month <- factor(
  factors_df$leaf_out_month, 
  levels = unique(factors_df$leaf_out_month[order(factors_df$leaf_out_doy)]))
```
...
lilacObs_sf$leaf_month <- factor(
  lilacObs_sf$leaf_month, levels = unique(factors_df$leaf_out_month))
```
...
pal_df <- data.frame(
  ...
  leaf_out_month = levels(factors_df$leaf_out_month))

# Join this data frame with the `factors_df` data frame
factors_df <- left_join(factors_df, pal_df, by = "leaf_out_month")
```
...
palRast <- colorFactor(
  ...
  domain = factors_df$leaf_out_doy,
...
function from the `leafem` package allows you to query specific pixels.
...
**Practice.** Try making some edits to change the map's appearance. For
